...
// SkillTemplate is the Claude Code project skill installed by `repowiki init`
// and `repowiki skill install`. It teaches an agent the wiki workflow.
const SkillTemplate = `---
name: repowiki
...
// InstallSkill writes the skill to .claude/skills/repowiki/SKILL.md under
// repoRoot. Returns "created", "updated", or "unchanged".
func InstallSkill(repoRoot string) (string, error) {
	dir := filepath.Join(repoRoot, ".claude", "skills", "repowiki")
	path := filepath.Join(dir, "SKILL.md")
	existing, err := os.ReadFile(path)
	if err == nil && string(existing) == SkillTemplate {
	...
	return result, os.WriteFile(path, []byte(SkillTemplate), 0o644)
}
